...
def main():
	if len(sys.argv) < 5:
		print("Error, parameters are missing: python run-ggplot2-single-plot.py path policy day_name type_of_screening [type_of_screening ...]")
		exit()
	...
	my_plot = (ggplot(df_plot) \
    	...
    	+ labs(title = "twelve classrooms (surgical mask, ventilation)", x = 'day', y = 'population', color = 'Agent type', linetype = 'type_of_screening_pretty')) \
	...
	my_plot.save('../../plot-ggplot2/' + long_path + '/' + policy + '/' + day_name + '/plot-' + policy + "_" + day_name, dpi=600)
	df_plot.to_csv('../../plot-ggplot2/' + long_path + '/' + policy + '/' + day_name + '/plot-file/plot-' + policy + "_" + day_name + '.csv')
main();
