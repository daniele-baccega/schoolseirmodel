...
def main():
	if len(sys.argv) < 5:
		print("Error, parameters are missing: python run-ggplot2-new-infected.py path policy day_name type_of_screening [type_of_screening ...]")
		exit()
	...
	while index < len(sys.argv):
		if str(sys.argv[index]) == "WithoutScreening":
			...
			paths.append(Path(__file__).parent / ("../../mean-results/" + long_path + "/" + policy + "/" + day_name + "/mean_" + policy + "_" + day_name + "_" + str(sys.argv[index]) + ".csv"))
	...
	my_plot = (ggplot(df_plot) \
    	...
    	+ geom_line() \
    	+ geom_vline(xintercept = [screening_day, screening_day+7, screening_day+14, screening_day+21, screening_day+28], linetype = 'dotted') \
    	+ labs(title = "twelve classrooms (surgical mask, ventilation)", x = 'day', y = 'new infected', color = 'Agent type', linetype = 'type_of_screening_pretty')) \
    	+ scale_color_manual(values=['#990000']) \
    	+ scale_x_continuous(breaks=[0, 5, 10, 15, 20, 25, 30, 35])
		
	my_plot.save('../../plot-ggplot2/' + long_path + '/' + policy + '/' + day_name + '/plot-' + policy + "_" + day_name + '_new-infected', dpi=600)
	df_plot.to_csv('../../plot-ggplot2/' + long_path + '/' + policy + '/' + day_name + '/plot-file/plot-' + policy + "_" + day_name + '_new-infected.csv')
main();
