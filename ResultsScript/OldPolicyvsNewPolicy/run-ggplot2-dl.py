...
def main():
	...
	paths.append(Path(__file__).parent / ("../../mean-results/OldPolicyvsNewPolicy/Prob-0.001/WholeQuarantined/" + policy + "/Wednesday/mean_" + policy + "_Wednesday.csv"))
	paths.append(Path(__file__).parent / ("../../mean-results/OldPolicyvsNewPolicy/Prob-0.001/OneQuarantined/" + policy + "/Wednesday/mean_" + policy + "_Wednesday.csv"))
	...
	my_plot = (ggplot(df_plot) \
		...
		+ geom_line() \
    	+ labs(title = "Old quarantine policy vs New quarantine policy (" + policy + " policy)", x = 'day', y = 'cumulative number of days', color = 'Distance learning type', linetype = 'Policy type')) \
	...
	my_plot.save('../../plot-ggplot2/OldPolicyvsNewPolicy/Prob-0.001/' + policy + '/dl.png', dpi=600)
	df_plot.to_csv('../../plot-ggplot2/OldPolicyvsNewPolicy/Prob-0.001/' + policy + '/dl.csv')
main();
